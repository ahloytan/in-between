...
$(document).ready(function() {
  ...
  $(".c2").click(function() {
    ...
    if (noOfCards >= minLimit) {
      ...
      $.each(allCards, function(key, value) {
        if (value[0] > posFCard && value[0] < posLCard) {
          cardsAvail += value[1];
        } //Only sum up winnable cards in range

        if (value[0] == posFCard || value[0] == posLCard) {
          sameCard += value[1];
        } //Only sum up same cards

      }); //Loop through whole JSON

      var winChance = (cardsAvail / (totalCards)) * 100;
      var sameCardChance = (sameCard / (totalCards)) * 100;
      var loseChance = 100 - winChance;

      $("#wStats").text("W: " + winChance.toFixed(1) + "% ");
      $("#lStats").text("| L: " + loseChance.toFixed(1) + "% | ");
      $("#sStats").text("Same: " + sameCardChance.toFixed(1) + "%");
  ...
  //card 3
  $(".c3").click(function() {
    var oCardVal = $(this).text();
    var posOCard = allCards[oCardVal];
  ...
  $('.c1, .c2, .c3').click(function(){
    ...
    showAllStats(allCards)
  }) //end of either of 3 cards clicked

}) //End of document ready

function showAllStats(allCards){
  $('#oStats').text('');
  $.each(allCards, function( key, value ) {
